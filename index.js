...
app.get('/list', (req, res) => {
  const Sql = "SELECT id,title, user_id, DATE_FORMAT(update_date, '%Y-%m-%d') AS update_date, DATE_FORMAT(update_date, '%Y-%m-%d') AS date  FROM board";
...
app.post('/delete', (req, res) => {
  ...
  const Sql = "DELETE from board WHERE id in (?)";
  db.query(Sql, [id], function(err, result) {
      if (err) throw err;
    res.send(result);
  });
